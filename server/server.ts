...
const campanhas = async (id: string | null = null) => {
  ...
  if (id) {
    ...
    const campanhasAgregadas = await Promise.all(
      campanhas.map(async (campanha: any) => {
        ...
        const alimentosIds = alimentosCampanha.map(
          (alimento: any) => alimento.alimento_id
        );
        ...
        const minutosRestantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) / (1000 * 60)
        );
        const horasRestantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) / (1000 * 60 * 60)
        );
        const diasRestantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
        );
        const mesesRestantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) /
            (1000 * 60 * 60 * 24 * 30)
        );
        const anosRestantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) /
            (1000 * 60 * 60 * 24 * 365)
        );
        ...
        const alimentosComDetalhes = alimentosCampanha.map(
          (alimentoCampanha: any) => {
    ...
    const campanhasComCamposCalculados = await Promise.all(
      campanhas.map(async (campanha: any) => {
        const now = new Date();
        const dtEncerramento = new Date(campanha.dt_encerramento_campanha);

        // Calcula o tempo restante em diferentes unidades
        const minutos_restantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) / (1000 * 60)
        );
        const horas_restantes = Math.floor(
          (dtEncerramento.getTime() - now.getTime()) / (1000 * 60 * 60)
        );
        ...
        const meses_restantes =
          (dtEncerramento.getFullYear() - now.getFullYear()) * 12 +
          (dtEncerramento.getMonth() - now.getMonth());
        ...
        const alimentosIds = alimentosCampanha.map(
          (alimento: { alimento_id: any }) => alimento.alimento_id
        ); // Supondo que alimento_id exista em alimento_campanha
        const detalhesAlimentos = await prisma.alimento.findMany({
          where: { id: { in: alimentosIds } },
        });
        ...
        return {
          ...
          qt_total_campanha: alimentosCampanha.reduce(
            (sum: any, alimento: any) => sum + alimento.qt_alimento_meta,
            0
          ),
          qt_doacoes_campanha: doacoes.reduce(
            (sum: any, doacao: any) => sum + doacao.qt_alimento_doado,
            0
          ),
          alimentos: alimentosComDetalhes,
        };
...
const insertUsuario = async (userInfos: any) => {
  ...
  if (userInfos.tipo_usuario === "pf") {
    usuario = {
      ...
      dt_nascimento_usuario: new Date(
        userInfos.dt_nascimento_usuario
      ).toISOString(),
